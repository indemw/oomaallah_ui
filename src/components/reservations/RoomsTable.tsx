            ...
            <TableBody>
              {data?.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>{r.room_types?.name} ({r.room_types?.code})</TableCell>
                  <TableCell>{r.floor || "-"}</TableCell>
                  <TableCell className="capitalize">{r.status}</TableCell>
                  <TableCell>{r.active ? "Yes" : "No"}</TableCell>
                  <TableCell className="text-right space-x-2">
                    <Button variant="outline" size="sm" onClick={() => { setEditing({ ...r, room_type_id: r.room_types?.id }); setOpen(true); }}>Edit</Button>
