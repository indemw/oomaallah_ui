...
-- Policies for menu_item_components
CREATE POLICY IF NOT EXISTS "Restaurant can view components"
...
CREATE POLICY IF NOT EXISTS "Restaurant can manage components"
ON public.menu_item_components
FOR ALL
USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Restaurant can view movements"
...
CREATE POLICY IF NOT EXISTS "Restaurant can insert movements"
ON public.stock_movements
FOR INSERT
WITH CHECK (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "Managers can update movements"
ON public.stock_movements
FOR UPDATE
USING (
  EXISTS (
...
-- Make sure the function owner is the same as the table owner for privileges
